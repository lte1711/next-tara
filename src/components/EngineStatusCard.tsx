...
export function EngineStatusCard({ engine, loading }: EngineStatusCardProps) {
  if (loading) {
    return (
      <div className="bg-panel rounded-lg p-6 border border-border-subtle">
        <h2 className="text-xl font-bold mb-4 text-text-strong">
          Engine Status
        </h2>
  ...
  const killSwitchStyle = {
    color: engine.kill_switch_on ? "var(--alert-critical)" : "var(--health-ok)",
    backgroundColor: engine.kill_switch_on
      ? "color-mix(in srgb, var(--alert-critical) 12%, transparent)"
      : "color-mix(in srgb, var(--health-ok) 12%, transparent)",
  };
  ...
  return (
    <div className="bg-panel rounded-lg p-6 border border-border-subtle">
      ...
      <div className="mb-4 p-3 rounded" style={killSwitchStyle}>
        <div className="flex items-center gap-2">
          <div
            className="w-3 h-3 rounded-full"
            style={{ backgroundColor: killSwitchStyle.color }}
          ></div>
          <span
            className="font-mono text-sm"
            style={{ color: killSwitchStyle.color }}
          >
