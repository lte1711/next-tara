...
export default function OpsPage() {
  ...
  const [pmxData, setPmxData] = useState<Record<string, unknown> | null>(null);
  ...
  useEffect(() => {
    const ws = new WebSocket("ws://127.0.0.1:8100/ws/events");
    ws.onmessage = (ev) => {
      try {
  ...
  // PROFITMAX v1 runner 상태 5초 폴링
  useEffect(() => {
    const fetchPmx = async () => {
      try {
        const r = await fetch("/api/profitmax/status?limit=20", { cache: "no-store" });
        if (r.ok) setPmxData(await r.json());
      } catch { /* ignore */ }
    };
    fetchPmx();
    const iv = setInterval(fetchPmx, 5000);
